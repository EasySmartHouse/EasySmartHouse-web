...
net.easysmarthouse.lang = {
    ...
    survey: function(selector, callback) {
        var urlLocale = this._getUrlParameter('locale');
        var oldvalue = '';
        switch (urlLocale) {
            case 'ru':
                oldvalue = 'ru_RU';
...
$(document).ready(function() {
    net.easysmarthouse.lang.survey('.bfh-languages > input[type=hidden]',
            function(updated) {
                var key = 'locale';
                var val = updated.split('_')[0];
                window.location = net.easysmarthouse.lang.updateQueryString(key, val);
            });
});
